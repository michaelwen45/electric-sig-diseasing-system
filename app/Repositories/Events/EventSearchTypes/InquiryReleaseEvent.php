...
class InquiryReleaseEvent extends EventSearchType
{
    var $limit = 50;
    var $eventType = 'InquiryEvent';
    var $eventName = 'InquiryReleaseEvent';

    function getEvents(){
        $retVals = [];
        $events = new InquiryClaimingEvent();
        $events = $events->whereIsRelease()->limit($this->getLimit())->get();
        foreach($events as $event){
            ...
            $actingUser = (!empty($actingUser) && $actingUser->exists)?($actingUser->userAccountInformation()->first()):(null);
            $actingUser = (!empty($actingUser))?($actingUser->toArray()):(null);
            $data = array();
